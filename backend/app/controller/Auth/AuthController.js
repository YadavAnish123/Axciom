...
const  editReminder=asynchandler(async(req,res)=>{
	try {
		console.log("log is calling")
		const {selectedDate,selectedSubject,description,emailAddress,contactNo,smsNo,recurringDays,_id}=req.body
		const updatedFields = {};
        ...
        if (selectedSubject) updatedFields.selectedSubject = selectedSubject;
        if (description) updatedFields.description = description;
        if (emailAddress) updatedFields.emailAddress = emailAddress;
        if (contactNo) updatedFields.contactNo = contactNo;
        if (smsNo) updatedFields.smsNo = smsNo;
        if (recurringDays) updatedFields.recurringDays = recurringDays;
...
const  setReminder=asynchandler(async(req,res)=>{
	try {
		...
		const {selectedDate,selectedSubject,description,emailAddress,contactNo,smsNo,recurringDays,createdby}=req.body
...
const  getrem=asynchandler(async(req,res)=>{
	try {
		console.log("getdata call")
		 ...
		 const {_id}=req.body;
		 //console.log(_id)
		 const d=Remind.find({createdby:'6526ab6e7e4fed79ee8e74fe'});
